...
function DisplayImage() {
  ...
  return (
    <div className='h-screen flex justify-center items-center relative'>
      <button onClick={() => deleteImage(images.length > 0 ? images[imagename].public_id : '')} className='bg-red-500 text-white font-semibold cursor-pointer w-28 h-8 absolute top-5 right-5 sm:top-10 sm:right-52 flex justify-center items-center'>
        {!buttonLoading ? 'Delete Image' :
          <div role="status">
      ...
      <img src={images.length > 0 ? images[imagename].secure_url : ''} alt="image" className='w-[90%] h-3/4 sm:w-3/4 sm:h-2/3 object-fill' />
    </div>
  )
}
